  ...
  onLoad() {
    ...
    if (!mbMap.getLayer('dv_lines_outline_highlight')) {
      mbMap.addLayer(
        {
          filter: ['all'],
          id: 'dv_lines_outline_highlight',
          layout: { visibility: 'visible' },
          paint: {
            'line-color': 'rgba(235,0,0, 1)', // rgba(195, 239, 239, 1)',
            'line-width': [
              'case',
              ['boolean', ['feature-state', 'hover'], false],
              2, // (this.get('routeType') === 'day' ? 3 : 4),
              0,
            ],
            'line-offset': [
              'case',
              ['boolean', ['feature-state', 'hover'], false],
              0, // (this.get('routeType') === 'day' ? 3 : 4),
              0,
            ],
          },
