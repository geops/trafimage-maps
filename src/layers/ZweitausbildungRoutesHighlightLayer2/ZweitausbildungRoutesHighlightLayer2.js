 ...
 */
class ZweitausbildungRoutesHighlightLayer2 extends VectorLayer {
   ...
   */
  load() {
    fetch('https://maps.style-dev.geops.io/data/base_v3.json')
      .then((data) => data.json())
      .then(() => {
        this.features = [];
      });
  ...
  populate() {
    ...
    for (let i = 0; i < this.features.length; i += 1) {
      ...
      const label = ZweitausbildungRoutesHighlightLayer2.generateLabel(feature);
    ...
    return fetch(
      ...
      .then((data) => {
        ...
        for (let i = 0; i < feats.length; i += 1) {
          const label = ZweitausbildungRoutesHighlightLayer2.generateLabel(
            feats[i],
          );
...
export default ZweitausbildungRoutesHighlightLayer2;
